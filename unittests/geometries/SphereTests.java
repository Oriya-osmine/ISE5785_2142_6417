...
class SphereTests {
    ...
    void testFindIntersections() {
        ...
        assertNull(sphere10.findIntersections(new Ray(new Point(0, 2, 2), new Vector(0, 0, -1))),
                "Ray outside sphere10 should not intersect");

        // TC02: Ray starts before and goes through the sphere10 (2 points)
        List<Point> result = sphere10.findIntersections(new Ray(new Point(0, 0, 3), new Vector(0, 1, -3)));
        assertNotNull(result, "Ray should intersect the sphere10");
        assertEquals(new Point(0,1,0), result.get(0), "Ray should intersect in  z=-1");
        assertEquals(new Point(0,0.8,0.6), result.get(1), "Ray should intersect in z=1");

        // TC03: Ray starts inside the sphere10 and exits (1 point)
        result = sphere10.findIntersections(new Ray(new Point(0, 0, 0.5), new Vector(0, 1, 2)));
        assertNotNull(result, "Ray from inside sphere10 should intersect");
        assertEquals(new Point(0,0.2358898943540673,0.9717797887081345), result.get(0), "Ray should intersect in z=1 ");

        // TC04: Ray starts after the sphere10 and goes away (no intersection)
        assertNull(sphere10.findIntersections(new Ray(new Point(0, 0, 2), new Vector(-1, -1, -1))),
        ...
        result = sphere10.findIntersections(new Ray(new Point(0, -1, 0), new Vector(0, 1, 1)));
        assertNotNull(result, "Ray from inside should intersect once");
        assertEquals(new Point(0,0,1), result.get(0));

        // TC11: Ray starts after the sphere10 on the same line
        assertNull(sphere10.findIntersections(new Ray(new Point(0, 0, -1), new Vector(0, 1, -1))),
        ...
        assertNull(sphere10.findIntersections(new Ray(new Point(0, 2, 0), new Vector(0, 0, 1))),
                "Orthogonal ray from beyond radius should not intersect");

        // TC111: Ray orthogonal to radius, starts at distance < radius
        result = sphere10.findIntersections(new Ray(new Point(0, 0.5, 0), new Vector(0, 0, 1)));
